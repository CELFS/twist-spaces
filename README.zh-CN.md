...
2026-08-28 当前界面与操作检查：

- 60 项 Swift 测试全部通过。新增 11 项覆盖激活/新建分离、首次启动、逐组新建、明确失败、菜单命令匹配、比例兼容、勾选不启动及图标点击区域；原有 49 项测试保留。
- debug 应用已沿用固定开发签名构建并通过严格签名校验；旧进程从应用菜单退出并确认停止后，已打开新版应用。
- 真实界面已显示独立激活/新建图标按钮和比例预览；勾选与取消勾选卡片会更新两个批量按钮，不会启动应用。编辑器滑块已实测从 50:50 更新至 55:45，测试草稿随后取消，未保存。
