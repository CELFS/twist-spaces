...
- 单组打开及批量打开会启动或激活保存的应用。启动前检查整个批次，应用缺失时阻止批次；重复应用每批只打开一次。此操作不打开指定项目，也不应用布局。
...
3. 从菜单栏或「显示布局面板」打开展示面板，勾选组合后「批量打开」，也可点击单行箭头打开该组合的应用。
...
- 49 项 Swift 测试全部通过，包含 50 次草稿创建/取消、无需窗口识别的保存、批量启动预检/去重/错误处理、中英文资源一致性、已安装应用与别名枚举、稳定选择标识、面板位置、宽度迁移及原生毛玻璃配置。
- debug 应用已沿用固定开发签名构建。
